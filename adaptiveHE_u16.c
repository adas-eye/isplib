...
#define MAX 65536
static void getRegionAccumHist(uint16* in, uint32 *accumHist, int wid, int hgt, int win_size, int limit)
{
	int i, j, k, l;
	uint16 *regionImg;
	...
	regionImg = (uint16*)calloc(regionLen, sizeof(uint16));
	...
	for (i=0;i<hgt;i+=win_size)
	{
		for (j=0;j<wid;j+=win_size)
		{
			...
			getHistogram_U16(regionImg, hist, regionLen);
			limitedHist_U16(hist, hist, limit, 0, true);
			accumHistogram_U16(hist, accumHist + gridIndex*MAX);
			gridIndex++;
		}
	}
...
static uint32 getHistGrid(uint16 val, int gridX, int gridY, uint32 *accumHist,
...
void adaptiveHE_U16(uint16 *in, uint16* in4hist, uint16 *out, int wid, int hgt, int win_size, int limit)
{
	int i, j;
	uint32 *accumHist;
